...
class Recipe extends Component {
  constructor () {
    ...
    this.handleConfirmDeleteOverlay = this.handleConfirmDeleteOverlay.bind(this);
    ...
    firebase.database().ref(`/recipes/${recipeKey}`).once('value', snap => {
      ...
      if (recipe.favoritedBy) {
        for (let key in recipe.favoritedBy) {
          if (this.props.user && recipe.favoritedBy[key] === this.props.user.uid) {
            this.setState({
              fav: true
            });
          }
  ...
  render () {
    let ingredients = this.state.recipe ? this.state.recipe.ingredients.map((ingredient, index) => {
      return <li key={index}>{ingredient}</li>
    }) : (
      <li>No ingredients have been added</li>
    );

    let methodSteps = this.state.recipe ? this.state.recipe.methodSteps.map((method, index) => {
    ...
    let addFavorite = (this.state.recipe && this.props.user) && (this.state.recipe.owner.uid !== this.props.user.uid) && !this.state.fav ? (
      <AddFavorite props={this.props} handleClick={this.handleAdd} />
    ) : null;

    let removeFavorite = (this.state.recipe && this.props.user) && (this.state.recipe.owner.uid !== this.props.user.uid) && this.state.fav ? (
      <RemoveFavorite recipeKey={this.props.match.params.id} userID={this.props.user.uid} handleRemove={this.handleRemove} />
    ) : null;
      ...
      <div className="overlay">
        <div className="box">
          ...
          <div className="box-buttons">
            <button onClick={() => this.setState({confirmOverlay: false})}>Cancel</button>
