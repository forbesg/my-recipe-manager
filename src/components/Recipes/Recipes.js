...
class Recipes extends Component {
          ...
          snap.forEach((recipe) => {
            ...
            if (recipe.image && !recipe.thumbnail && !recipe.image.remote) {
              ...
              let imageRef = firebase
                .storage()
                .ref("/recipes")
                .child(`${key}/thumbnail/${thumbnailFileName}`);
  ...
  render() {
    let recipes = this.state.filteredRecipes
      ...
      : null;
    let cuisineFilter = this.state.cuisines
      ? this.state.cuisines.map((cus, index) => {
          let flag = require(`../../images/flags/${cus.flag}.svg`).default;
          return (
            <div
    ...
    let recipesElement = this.state.recipes ? (
      <div className="recipes">
        ...
        <div className="cards">{recipes}</div>
      </div>
    ) : (
      <Loader />
    );
